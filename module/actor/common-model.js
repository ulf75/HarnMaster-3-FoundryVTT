 ...
 * @mixin
 */
export class CommonTemplate extends ActorDataModel.mixin(CurrencyTemplate) {
    /** @inheritDoc */
    static defineSchema() {
        ...
        for (const [id, abl] of Object.entries(this.abilities)) {
            ...
            abl.save.toString = function () {
                foundry.utils.logCompatibilityWarning(
                    "The 'abilities.<ability>.save' property is now stored in " + "'abilities.<ability>.save.value'.",
                    {since: '4.3', until: '4.5'}
                );
    ...
    calculateAbilityCheckProficiency(multiplier, ability) {
        ...
        if (multiplier < 1) {
            if (this.parent._isRemarkableAthlete(ability)) {
                multiplier = 0.5;
                roundDown = false;
            } else if (this.parent.flags.dnd5e?.jackOfAllTrades) multiplier = 0.5;
        }
