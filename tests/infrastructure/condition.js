...
export class ConditionTestCase extends BaseTestHM3 {
    ...
    async _test() {
        ...
        console.info('Adding condition: %s to token: %s', hm3.Condition.DISTRACTED, t.name);
        console.assert(d && d instanceof ActiveEffect, 'Condition is NOT an ActiveEffect: %O', d);
        const b = await t.addCondition(hm3.Condition.BERSERK);
        console.assert(b && b instanceof ActiveEffect, 'Condition is NOT an ActiveEffect: %O', b);
        console.info('Adding condition: %s to token: %s', hm3.Condition.BERSERK, t.name);
        const u = await t.addCondition(hm3.Condition.UNCONSCIOUS);
        console.info('Adding condition: %s to token: %s', hm3.Condition.UNCONSCIOUS, t.name);
        console.assert(u && u instanceof ActiveEffect, 'Condition is NOT an ActiveEffect: %O', u);
        console.assert(t.hasCondition(hm3.Condition.PRONE), 'Combatant is NOT prone: %O', t);
        const s = await t.addCondition(hm3.Condition.SECONDARY_HAND);
        console.info('Adding condition: %s to token: %s', hm3.Condition.SECONDARY_HAND, t.name);
